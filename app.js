...
function isRedirectOnlyItem(item) {
  return item.kind === 'citation' || item.kind === 'surface' || item.kind?.startsWith('heuristic');
}

function isFullEntryItem(item) {
  return item.kind === 'lemma' || item.kind === 'lexicalized-lemma';
}
...
function renderResultCard(item, index) {
  ...
  const isRedirect = isRedirectOnlyItem(item);

  if (isRedirect) {
    return `
      <div class="result-item redirect-card" ${index === 0 ? 'data-autoselect="true"' : ''}>
        <div class="result-top">
          <div>
            <div class="result-headword">${escapeHtml(item.surface || entry.headword)}</div>
            <div class="muted">${escapeHtml(entry.pos)}</div>
          </div>
          <span class="pill primary">${escapeHtml(item.type)}</span>
        </div>
        <div class="redirect-message">${escapeHtml(item.surface || entry.headword)} 是 <a href="#" class="lemma-link" data-lookup="${escapeHtml(entry.headword)}">${escapeHtml(entry.headword)}</a> 的${escapeHtml(item.type)}。</div>
        <div class="redirect-action">请查看辞典形：<a href="#" class="lemma-link" data-lookup="${escapeHtml(entry.headword)}">${escapeHtml(entry.headword)}</a></div>
      </div>
    `;
  }
  ...
  return `
    <div class="result-item" data-open-entry="${escapeHtml(entry.headword)}" ${index === 0 ? 'data-autoselect="true"' : ''}>
      ...
      <div class="result-desc">${escapeHtml(analysisMessage(item))}</div>
    </div>
  `;
}
...
function searchEntries(query) {
  ...
  const firstFullEntry = merged.find((item) => isFullEntryItem(item));
  if (firstFullEntry) {
    renderEntryDetail(firstFullEntry.entry.headword, firstFullEntry);
  } else {
    els.entryDetailCard.classList.add('hidden');
  }
}
...
function renderEntryDetail(headword, preferredAnalysis = null, rawSurface = '') {
  ...
  const forms = (entry.forms || []).length
    ? entry.forms
        .map(
          (form) => `
            <div class="list-item form-block">
              <div class="term-line"><strong>${escapeHtml(form.surface)}</strong><span class="pill">${escapeHtml(form.type)}</span></div>
              ${form.example ? renderExampleCard(form.example) : '<div class="muted leading">待补这个形式的例句。</div>'}
            </div>
          `
        )
        .join('')
        ...
        .map(
          (item) => `
            <div class="list-item form-block">
              ...
              ${item.meaning ? `<div class="muted leading">${escapeHtml(item.meaning)}</div>` : ''}
  ...
  els.entryDetail.innerHTML = `
    <div class="entry-header">
      ...
      ${entry.lexicalized_from ? `<div class="footer-note rich-note">该词原本来自 <strong>${escapeHtml(entry.lexicalized_from.lemma)}</strong> 的<strong>${escapeHtml(entry.lexicalized_from.type)}</strong>，本站也把它作为常用独立词条收录。</div>` : ''}
    </div>
...
function bindEvents() {
  ...
  document.body.addEventListener('click', (event) => {
    ...
    if (tokenEl) {
      ...
      if (isRedirectOnlyItem(analyses[0])) {
        els.mainSearch.value = token;
        switchMode('lookup');
        searchEntries(token);
      } else {
        renderEntryDetail(analyses[0].entry.headword);
      }
    }
  });
